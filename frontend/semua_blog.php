...
while ($row = mysqli_fetch_object($result)) {
    if ($showId && $row->id == $showId) {
        $bigCard = $row;
    } elseif ($keyword && !$bigCard && (stripos($row->title, $keyword) !== false || stripos($row->content, $keyword) !== false)) {
        $bigCard = $row;
    } else {
        $smallCards[] = $row;
    }
...
<style>
body { background: #f4f6f9; font-family: 'Segoe UI', sans-serif; display:flex; flex-direction:column; min-height:100vh; }
.content { flex:1; }
...
.card { border-radius:14px; transition:transform 0.25s ease, box-shadow 0.25s ease; }
.card:hover { transform:translateY(-6px); box-shadow:0 10px 25px rgba(0,0,0,0.12); }
.card img { width:100%; object-fit:cover; border-radius:12px; }
.card-text { text-align:justify; font-size:14px; color:#555; }

footer { background:black; color:white; padding:17px 0; }
.footer a { color:#fff; text-decoration:none; }
.footer a:hover { color:#0d47a1; }
.footer i, .footer svg { color:#fff !important; fill:#fff !important; }

.btn-outline-primary { border-radius:10px; font-weight:500; transition:all 0.25s ease; }
.btn-outline-primary:hover { background:#0d47a1; border-color:#0d47a1; color:#fff; }
...
.input-group .clear-btn {
    ...
    border:none; background:transparent; display:none; cursor:pointer; z-index:10;
}
</style>
</head>
<body>
<div class="content">
    <div class="banner">
        <h2>Daftar Berita</h2>
        <p>Kumpulan artikel & informasi terbaru</p>
        <nav aria-label="breadcrumb" class="d-flex justify-content-center mt-3">
            <ol class="breadcrumb bg-transparent m-0 p-0">
                ...
                <li class="breadcrumb-item"><a href="./index.php#blogs" class="text-white">Berita</a></li>
    ...
    <div class="container my-4">
        ...
        <div class="d-flex justify-content-center my-4 position-relative">
            <form method="get" class="w-75 position-relative">
                <div class="input-group input-group-lg shadow-sm rounded-pill overflow-hidden position-relative">
                    <input type="text" name="keyword" id="searchInput" class="form-control border-0 ps-4" placeholder="Cari berita..." value="<?= htmlspecialchars($keyword) ?>">
        ...
        <div class="card mb-5 shadow-sm border-0 mx-auto" style="max-width:800px;">
            <?php if (!empty($bigCard->image)): ?>
                <img src="../storages/blogs/<?= $bigCard->image ?>" alt="<?= $bigCard->title ?>" class="img-fluid w-100" style="height:400px; object-fit:cover; border-top-left-radius:14px; border-top-right-radius:14px;">
            <?php endif; ?>
            <div class="card-body d-flex flex-column">
                <h5 class="card-title mb-2"><?= htmlspecialchars($bigCard->title) ?></h5>
                <p class="text-muted mb-2" style="font-size:14px;">
                    <i class="bi bi-calendar-event"></i> <?= !empty($bigCard->date) ? date("d M Y", strtotime($bigCard->date)) : "-" ?>
                    &nbsp; | &nbsp;
                    <i class="bi bi-person-circle"></i> <?= htmlspecialchars($bigCard->author ?? "-") ?>
                </p>
                <p class="card-text flex-grow-1"><?= nl2br(htmlspecialchars($bigCard->content)) ?></p>
            </div>
        </div>
        <?php endif; ?>
                ...
                <div class="col-md-6 col-lg-4 d-flex">
                    <div class="card h-100 border-0 shadow-sm d-flex flex-column w-100">
                        <?php if (!empty($item->image)): ?>
                            <img src="../storages/blogs/<?= $item->image ?>" alt="<?= $item->title ?>" class="img-fluid w-100" style="height:200px; object-fit:cover; border-top-left-radius:14px; border-top-right-radius:14px;">
                        <?php else: ?>
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <h5 class="card-title text-start"><?= htmlspecialchars($item->title) ?></h5>
                            <p class="card-text text-start flex-grow-1">
                                <?= substr(strip_tags($item->content),0,120) ?><?= strlen($item->content)>120 ? "..." : "" ?>
                            </p>
                            <div class="mt-auto">
...
function toggleClearBtn(){ clearBtn.style.display = searchInput.value.trim() ? "flex" : "none"; }
