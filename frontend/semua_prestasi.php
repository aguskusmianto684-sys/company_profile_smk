...
while ($row = mysqli_fetch_object($result)) {
    if ($showId && $row->id == $showId) {
        $bigCard = $row;
    } elseif ($keyword && !$bigCard && (stripos($row->title, $keyword) !== false || stripos($row->description, $keyword) !== false)) {
        $bigCard = $row;
    } else {
        $smallCards[] = $row;
    }
...
<body>
    <div class="content">
        <div class="banner">
            <h2>Daftar Prestasi Siswa/i SMKN 3 Banjar</h2>
            <p>Pencapaian terbaik siswa dalam berbagai bidang</p>
            <nav aria-label="breadcrumb" class="d-flex justify-content-center mt-3">
                <ol class="breadcrumb bg-transparent m-0 p-0">
                    ...
                    <li class="breadcrumb-item"><a href="./index.php#achievements" class="text-white">Prestasi</a></li>
                    <li class="breadcrumb-item active text-white"><a href="./index.php#achievements" class="text-white">Kontak</a></li>
                </ol>
            </nav>
        </div>
            ...
            <div class="d-flex justify-content-center my-4 position-relative">
                <form method="get" class="w-75 position-relative">
                    <div class="input-group input-group-lg shadow-sm rounded-pill overflow-hidden position-relative">
                        <input type="text" name="keyword" id="searchInput" class="form-control border-0 ps-4" placeholder="Cari prestasi..." value="<?= htmlspecialchars($keyword) ?>">
            ...
            <?php if ($bigCard): ?>
                <div class="card mb-5 shadow-sm border-0 mx-auto" style="max-width:800px;">
                    <?php if (!empty($bigCard->image)): ?>
                        <img src="../storages/achievements/<?= $bigCard->image ?>" alt="<?= htmlspecialchars($bigCard->title) ?>" class="img-fluid w-100" style="height:400px; object-fit:cover; border-top-left-radius:14px; border-top-right-radius:14px;">
                    <?php endif; ?>
                    ...
                    <div class="col-md-6 col-lg-4 d-flex">
                        <div class="card h-100 border-0 shadow-sm d-flex flex-column w-100">
                            <?php if (!empty($item->image)): ?>
                                <img src="../storages/achievements/<?= $item->image ?>" alt="<?= htmlspecialchars($item->title) ?>" class="img-fluid w-100" style="height:200px; object-fit:cover; border-top-left-radius:14px; border-top-right-radius:14px;">
                            <?php else: ?>
                            ...
                            <div class="card-body d-flex flex-column">
                                <div class="d-flex justify-content-start align-items-center text-muted mb-2" style="font-size:13px;">
                                    <i class="bi bi-calendar-event me-1"></i> <?= !empty($item->date) ? date("d M Y", strtotime($item->date)) : "-" ?>
                                ...
                                <p class="card-text text-start flex-grow-1"><?= substr(strip_tags($item->description), 0, 120) ?><?= strlen($item->description) > 120 ? "..." : "" ?></p>
                                <div class="mt-auto">
